...
function initTradingViewChart() {
  ...
  // Compatibilidad garantizada para Lightweight Charts v4 y v5
  if (typeof state.chartInstance.addCandlestickSeries === 'function') {
    ...
    // Fallback automático para v5 si se carga la versión superior
    state.candlestickSeries = state.chartInstance.addSeries(LightweightCharts.CandlestickSeries, {
      upColor: '#10b981',     // Emerald 500
      downColor: '#f43f5e',   // Rose 500
      borderVisible: false,
      wickUpColor: '#10b981',
    ...
    state.volumeSeries = state.chartInstance.addSeries(LightweightCharts.HistogramSeries, {
      color: '#3b82f6', // Blue 500
      priceFormat: {
        type: 'volume'
      },
...
async function fetchYahooFinanceData(ticker, range, interval) {
  ...
  const cleanTicker = ticker.replace('.BA', '');
  ...
  // Definición de proveedores y proxies en orden de prioridad
  // Eliminamos cabeceras custom para evitar preflight OPTIONS que bloquea CORS en GitHub Pages
  const providers = [
    {
      name: 'Yahoo Directo',
      buildUrl: () => yahooUrl,
    ...
    {
      name: 'Alpha Vantage Demo (US Tickers)',
      buildUrl: () => `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${cleanTicker}&apikey=demo`,
      parse: async (resp) => {
        ...
        if (timeSeries) {
          ...
          for (const dt of dates) {
            timestamps.push(Math.floor(new Date(dt).getTime() / 1000));
            const bar = timeSeries[dt];
            opens.push(parseFloat(bar["1. open"]));
            highs.push(parseFloat(bar["2. high"]));
            lows.push(parseFloat(bar["3. low"]));
            closes.push(parseFloat(bar["4. close"]));
            volumes.push(parseInt(bar["5. volume"]));
          }
        ...
        throw new Error('Alpha Vantage sin datos para ' + ticker);
      }
    },
    {
  ...
  for (const provider of providers) {
    try {
      const proxyUrl = provider.buildUrl(yahooUrl);
      console.log(`[Procesando: ${provider.name}] Intentando ${ticker}...`);

      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 8000); // 8 seg timeout por intento

      const response = await fetch(proxyUrl, {
        signal: controller.signal
        // Sin cabeceras custom para evitar disparar preflight OPTIONS CORS
      });
      ...
      if (data?.chart?.result?.[0]) {
        ...
        if (result.timestamp && result.timestamp.length > 0 &&
          ...
          console.log(`[Éxito: ${provider.name}] ✅ Datos reales cargados para ${ticker} — ${result.timestamp.length} velas`);
          saveToSessionCache(cacheKey, cacheTimeKey, result);
          state.connectionStatus = 'connected';
          updateConnectionIndicator();
          return result;
        } else {
          throw new Error('Estructura sin datos de velas');
        }
      }
      ...
      console.warn(`[Falló: ${provider.name}] ❌ ${ticker}: ${isAbort ? 'TIMEOUT (8s)' : e.message}`);
      lastError = e;
    }
  }

  throw new Error(`Todos los proveedores y proxies fallaron para ${ticker}. Último error: ${lastError?.message}`);
}
